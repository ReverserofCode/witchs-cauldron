# AGENTS.md - 프로젝트 컨텍스트 (루트)

이 문서는 Codex가 프로젝트를 빠르게 이해하고 효율적으로 작업할 수 있도록 모든 핵심 정보를 통합한 문서입니다.
...
│
├── AGENTS.md                         # 이 문서 (통합 컨텍스트)
├── docker-compose.yml                # 개발 환경 (Frontend + Backend)
├── docker-compose.prod.yml           # 프로덕션 환경
│
├── .codex/                           # Codex 프로젝트 에이전트 설정
│   └── agents/*.toml                 # 역할별 지침 16개
│
...
| 에이전트 | 용도 |
|----------|------|
| `api-contract-checker` | Frontend/Backend API 계약 검증 |
| `code-generator` | 코드 생성·수정·리팩터링 |
| `code-tester` | 테스트·빌드 검증 |
| `code-validator` | 코드 품질·보안 리뷰 |
| `codebase-structure-analyzer` | 코드베이스 구조 분석 |
| `dependency-analyzer` | 의존성·취약점 분석 |
| `documentation-generator` | 문서 생성·업데이트 |
| `external-tool-integrator` | 외부 서비스 연동 |
| `image-asset-generator` | 이미지 자산 생성 |
| `lint-checker` | ESLint·TypeScript 검사 |
| `orchestrator-planner` | 멀티 에이전트 조율 |
| `release-note-writer` | 릴리스 노트 작성 |
| `requirements-analyzer` | 요구사항 분석 |
| `smoke-test-runner` | 비파괴 런타임 점검 |
| `token-optimizer` | 컨텍스트 최적화 |
| `workflow-guardian` | GitHub Actions 안전성 검토 |

에이전트 지침은 `.codex/agents/*.toml`에 두며, 모델은 실행 환경 설정을 상속합니다. 프로젝트 컨텍스트의 기준 문서는 루트 `AGENTS.md`입니다.
...
### 운영 서버 직접 접속 지침
- SSH 비밀번호는 저장소 파일, Markdown 문서, 명령줄 인자에 직접 쓰지 않습니다.
- 로컬 직접 접속은 `.secrets/prod-ssh-password.secure.txt` 파일을 사용합니다. `.secrets/`는 Git 추적 제외 대상입니다.
- `python -m pip install -r tools/requirements.txt`로 로컬 도구 의존성을 설치합니다.
- 비밀번호 파일은 PowerShell에서 `Read-Host "SSH password" -AsSecureString | ConvertFrom-SecureString | Set-Content -NoNewline .secrets/prod-ssh-password.secure.txt`로 생성합니다.
- 서버 명령은 `tools/Invoke-ProdSsh.ps1`를 사용합니다.
  - 예: `.\tools\Invoke-ProdSsh.ps1 -UserName <ssh-user> -DeployPath <server-project-path> -RemoteCommand "pwd && docker ps"`
- 서버 공개키 fingerprint를 별도 채널로 확인해 `known_hosts`에 등록하며, 미등록 또는 변경된 키는 연결을 거부합니다.
- 접속 사용자명/포트/배포 경로/known-hosts 경로는 `WITCHS_SSH_USERNAME`, `WITCHS_SSH_PORT`, `WITCHS_DEPLOY_PATH`, `WITCHS_SSH_KNOWN_HOSTS` 환경 변수로 전달할 수 있습니다.

### 3) 클립 렌더링 정책
- 운영 환경은 Backend가 수집한 클립을 `shared_clips` 볼륨으로 Frontend에 제공합니다.
- 권리와 용량을 확인한 선별 정적 클립만 `frontend/public/clips/`에 Git으로 포함할 수 있습니다.
...
### 5) Codex 모델 확인 방법
- `.codex/agents/*.toml`에서 역할별 지침을 확인합니다.
- 모델은 저장소에 고정하지 않고 Codex 실행 환경 설정을 상속합니다.
