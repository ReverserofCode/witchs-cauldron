...
describe("clip catalog", () => {
  it("sorts video files by modification time and exposes their original title", () => {
    ...
    expect(clips[0]).toMatchObject({
      title: "마녀의 원본 클립 이름",
      src: "/media/clips/clip_newer456_%EB%A7%88%EB%85%80%EC%9D%98-%EC%9B%90%EB%B3%B8_%ED%81%B4%EB%A6%BD_%EC%9D%B4%EB%A6%84.mp4?v=2000",
    });
  ...
  it("keeps the first file when clip directories contain the same filename", () => {
    const clips = buildClipCatalog([
      { name: "clip_duplicate_공유본.mp4", mtimeMs: 1_000 },
      { name: "clip_duplicate_공유본.mp4", mtimeMs: 9_000 },
    ]);

    expect(clips).toHaveLength(1);
    expect(clips[0].src).toBe(
      "/media/clips/clip_duplicate_%EA%B3%B5%EC%9C%A0%EB%B3%B8.mp4?v=1000"
    );
  });
