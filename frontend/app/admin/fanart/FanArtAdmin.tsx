...
export default function FanArtAdmin() {
  ...
  const canUpload = editable && selected.permission.display && selected.permission.resize && selected.permission.credit && selected.permission.confirmedAt && selected.permission.evidence.trim() && selected.review.creatorConfirmedAt && selected.review.confirmedAt && selected.review.status === "confirmed_non_generative";

  return <main className="mx-auto max-w-6xl px-4 py-10 text-purple-950">
    <div className="mb-6 flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-2xl font-bold">팬아트 검수함</h1><p className="mt-2 text-sm">작품별 허락과 검수 후 게시합니다. 자동 수집·메시지 발송은 하지 않습니다.</p></div><a href="/admin/analytics" className="btn">방문 분석</a></div>
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)]">
      ...
      <section className="min-w-0 rounded-2xl border border-purple-200 bg-white p-5">
        {!selected ? <p>작품을 등록하거나 목록에서 선택해 주세요.</p> : <div className="space-y-6">
          ...
          <ReviewForm key={`${selected.id}:${selected.version}`} work={selected} disabled={busy || !editable} onSave={input => run(async () => {
          ...
          <div className="space-y-3"><h3 className="font-semibold">4. 게시 및 철회</h3><div className="flex flex-wrap gap-2">
            <button className="btn btn-primary" disabled={busy || selected.status !== "ready"} onClick={() => void action("publish", "작업실에 게시했습니다.")}>게시 승인</button>
