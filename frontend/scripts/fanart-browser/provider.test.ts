...
import { createCafeBrowserProvider } from "./provider.mjs";
...
type Options = { author?: string; board?: string; account?: string; extra?: string; noConfirmation?: boolean; collapseText?: boolean; comments?: Comment[] };
let browser: Browser;
...
async function fixture(options: Options = {}) {
  ...
  await page.route("**/*", async route => {
    ...
    if (request.url() !== URL) { await route.abort(); return; }
    await route.fulfill({ contentType: "text/html; charset=utf-8", body: `<!doctype html><html><head><meta charset="utf-8"><style>.text { white-space: ${options.collapseText ? "normal" : "pre-wrap"}; }</style></head><body>
      <a class="signed-in" href="/ca-fe/cafes/30182989/members/${options.account ?? "operator-key"}">operator</a>
      <article><h1>대장장이 김모잉</h1>
      ...
      <textarea aria-label="댓글"></textarea><button class="submit">등록</button>
  ...
  const provider = createCafeBrowserProvider({ page, selectors, operatorMemberKey: "operator-key", timeoutMs: 500 });
...
describe("cafe browser provider against intercepted local DOM", () => {
  ...
  it("reports an uncertain remote result without clicking twice", async () => {
    ...
    try {
      await expect(f.provider.sendRequest(URL, "request", "[moingfans:test]")).rejects.toThrow(/uncertain/i);
      expect(f.submissions()).toBe(1);
    } finally { await f.context.close(); }
  });
  ...
  it("rejects an iframe for a different article before inspecting its controls", async () => {
    const f = await fixture();
    try {
      await f.page.route(URL, route => route.fulfill({ contentType: "text/html", body: '<iframe id="cafe_main" src="https://cafe.naver.com/ca-fe/cafes/30182989/articles/99999"></iframe>' }));
