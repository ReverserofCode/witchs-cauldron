...
| `wc:broadcasts:last-visited:v1` | 다시보기 목록을 정상 조회한 직전 방문 ISO 시각 |

브라우저 데이터를 지우거나 다른 기기를 사용하면 시청 상태가 동기화되지 않습니다. YouTube API 키 누락이나 일시 장애로 목록을 불러오지 못한 방문은 마지막 방문 시각을 갱신하지 않으므로, 복구 후 새 방송 표시가 사라지지 않습니다. 저장소 접근이 차단되어도 다시보기 목록과 외부 링크는 계속 사용할 수 있습니다.
