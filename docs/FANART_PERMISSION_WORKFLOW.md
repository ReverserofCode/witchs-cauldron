...
- 일관된 백업은 관리자 변경/업로드를 중단한 상태에서 전용 테이블을 포함한 PostgreSQL 백업과 볼륨 스냅샷을 함께 생성한다. 복구 시 같은 시점의 DB/볼륨을 함께 되돌리고 UID/GID 1001 접근권한, 게시 이미지200·철회 이미지404를 확인한 다음 쓰기를 재개한다.
