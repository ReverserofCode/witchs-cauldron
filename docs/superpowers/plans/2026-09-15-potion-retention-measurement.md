...
- **구현:** 별도 이벤트 계약·전용 DB pool/테이블, ID 수명·집계 제외·활동 provider, 보호된 코호트 집계, `potion-profile`/`potion-retention` 명령과 수동 workflow 선택지를 구현했다. 일반 Analytics 계약·집계와 분리하며 지표·삭제 절차는 [측정 운영 가이드](../../POTION_RETENTION_MEASUREMENT.md)를 따른다.
- **기술 검증:** [CI 34965761454](https://github.com/ReverserofCode/witchs-cauldron/actions/runs/34965761454)가 `3420fdc`에서 성공했다. 일반 테스트의 172 통과/16 DB 보류와 별개로 폐기용 PostgreSQL16 테스트 29/29가 skip 없이 통과해 보류 항목을 모두 검증했다. 두 실행의 중복 테스트는 합산하지 않는다. 실제 provider·인증된 DB summary·회귀 smoke와 Node22/Docker/백엔드 검증도 통과했다.
- **병합과 배포 구분:** 게이지 좌표·DB fixture 보완 후 전체 브랜치 검토 승인을 받았고 [PR #6](https://github.com/ReverserofCode/witchs-cauldron/pull/6)은 `0483162`로 병합됐다. 운영 배포/스키마 설치 완료는 여기서 확정하지 않으며 [릴리스 기록](../../POTION_RELEASE_2026-09-15.md)의 실제 증거로 확인한다.
- **파일럿 미시작:** `POTION_PILOT_WINDOW=null`이며 ID 생성·전송·신규 수집은 비활성이다. 사람 대상 사용성 평가는 미실시이고 운영 담당자·T0/모집 종료/관찰 종료 날짜는 미정이다. 자동 정리 예약은 추가하지 않았으며 명시적 운영 DB·스키마·삭제 담당/승인·안내를 확인한 뒤 별도 운영 커밋으로 준비해야 한다. 기술 구현이나 CI 성공을 파일럿 효과 검증으로 간주하지 않는다.

아래 체크박스·기준 SHA·예약 예시는 최초 실행 계획을 보존한 것이다. 미체크만으로 현재 미구현을 뜻하지 않으며, 특히 Task 4의 nightly 예약은 아직 수행하지 않은 활성화 게이트다. 현재 완료/보류 판정은 이 절과 릴리스 증거를 따른다.
