...
FanArtWork uses camelCase: id, sourceUrl, sourceKey, title, credit, status, version, createdAt, updatedAt, publishedAt, withdrawnAt, requestedAt, permission ({display,resize,credit,confirmedAt,evidence}), review ({status,creatorConfirmedAt,confirmedAt,note}), asset ({key,sha256,bytes,width,height}|null), approvedHash (string|null). States candidate/requested/ready/published/withdrawn/rejected. Review states pending/confirmed_non_generative/unclear/excluded_generative. creatorConfirmedAt records the creator non-generative assertion independently of confirmedAt/operator review. Permission evidence must be nonempty before upload/publication; review.note remains optional to avoid unnecessary personal information. Asset replacement clears both creator and operator confirmation timestamps.
